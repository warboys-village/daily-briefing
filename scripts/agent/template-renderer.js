...
function renderPlanCard(item, villageName = 'Warboys') {
...
function renderFullBriefingHtml(data, villageName = 'Warboys', county = 'Cambridgeshire', villageConfig = {}) {
  let md = '';
  ...
  if (data.governance && data.governance.length > 0) {
    const councilName = villageConfig.parishCouncil || `${villageName} Parish Council`;
    ...
    const councilSource = (villageConfig.sources || []).find(s => s.type === 'parish-council' || s.type === 'town-council');
    const councilUrl = councilSource ? councilSource.url : 'https://www.warboysparishcouncil.gov.uk/the-council/meeting-calendar/?meetings_view-1=list';
    ...
    md += `  </div>\n`;
    md += `  <div class="briefing-block-content">\n\n`;
    md += `    <div class="governance-calendar-banner" style="background: var(--color-tag-bg); padding: 0.75rem 1rem; border-radius: 6px; margin-bottom: 1.25rem; font-weight: 600; font-size: 0.95rem;">📅 Official ${councilName} Meetings & Agendas: <a href="${councilUrl}" target="_blank" rel="noopener">${councilName} Meeting Calendar &rarr;</a></div>\n\n`;

    for (const [meetingHeading, mItems] of meetingsMap.entries()) {
      const docUrl = mItems[0] ? (mItems[0].documentUrl || mItems[0].url) : councilUrl;
